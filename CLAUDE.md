...
posed `world`/`lights`, a no-op if OpenMP is absent). `apply_post` (its blur passes +
quantize loop are likewise OpenMP-parallel over rows): blur →
...
- **`HitRecord` references its material, it doesn't copy it:** `rec.material` is a
  `const Material*` into the posed shape, and `rec.albedo` is the **final** surface
  color (`material->albedo` after any procedural texture / ring-ramp sampling). Read
  `rec.albedo` for the shaded color and `rec.material->{emissive,two_sided,atmosphere}`
  for flags — copying the whole `Material` (with its color-ramp vectors) per hit was a
  per-ray allocation cost. Shading code that used to read `rec.material.albedo` now
  reads `rec.albedo`.
- **Shadow rays skip texturing:** `hit()` / `hit_world` take a `bool shading` (default
  true); `ray_color` passes `false` for occlusion rays, so `Sphere::hit` early-returns
  before the procedural surface texture (it only needs geometry + the material pointer
  to test the emissive-transparency rule). The expensive noise is evaluated once, for
  the visible hit.
- **Procedural noise hashing is no-trig:** `sphere.cpp`'s `hash31`/`hash11` use an
  integer bit-mix (`mix32` over a ×4096-quantized coordinate), not `std::sin` — the
  texture path called the hash dozens of times per pixel, so this matters. Changing the
  mix reshuffles the exact lattice values (a seed renders differently), so don't swap it
  casually.
