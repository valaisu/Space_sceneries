...
- `post.h` / `post.cpp` — `PostProcess`, `DitherMode`; `generate_palette` (HSV harmonic ramp; 0/1/2 anchor colors + a `base_hue` rotation + seeded `randomness`) + `apply_post` (blur → quantize/dither over the RGBA8 buffer). In `core`, no UI deps.
...
2** base colors (`anchor_count`), rotated bodily by a `base_hue` slider (recenters the
whole ramp on any hue — green/blue/orange), with a seeded `randomness` jitter and a
